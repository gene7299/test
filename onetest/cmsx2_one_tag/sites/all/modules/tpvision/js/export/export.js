...
function planpageSchedulerCalculation(page_scheduler) {
                ...
                if ((st_time <= cu_time) && (so_time >= cu_time)) {
                    ...
                    if (found) {
                        ...
                        if (psch != key) {
                            ...
                            console.log(psch + '<---cancelling the previous page scheduler & running new page scheduler 1--->' + key);
    ...
    if (def) {
        ...
        key = "default";
        if (psch != key) {
            console.log(psch + '<---cancelling the previous page scheduler defaultc & running new page scheduler 2--->' + key);
                  ...
                  if ($("video#" + pl_item).length) {
                    ...
                    || navigator.userAgent.indexOf("BDL4550D") > -1) {
                      ...
                      if (isOldVideoLoad) {
                       ...
                       var html1 = '<video id="' + pl_item + '" style="' + css + '" muted><source src="' + src + '" type="video/mp4"><source src="' + src + '" type="video/ogg"><source src="' + src + '" type="video/mov"></video>';
 ...
 function checkingVideoObjectInCurrentPage(video, id){
    ...
    if ($("#"+vid+" .broadcast-object").length == 0) {
        if($("#"+vid+" video").length){
            ...
            var html1 = '<video id="#video-'+vid+'" style="' + css + '" muted><source src="' + src + '" type="video/mp4"></video>';
...
function ChangeVideoSourceOnPlayback(pl, pl_item, playlistItems, onid, sch, snid, pl_details, count, i, stop, currentPlaylist){
    ...
    if ($(pl).find('.' + pl_item).hasClass('video')) {
        var src = $(pl + " ." + pl_item).attr("src");
        var css = $(pl + " ." + pl_item).attr("css");
        var videoTag = '<video id="' + contentId + '-video-item" style="' + css + ';background-color:#000;object-fit:fill;" src="' + src + '" muted>';
